...
/**
 * LiquidEther — verbatim WebGL fluid simulation from the provided spec.
 * Renders an interactive, atmospheric fluid background using Three.js.
 */
export default function LiquidEther({
  ...
  const webglRef = useRef<{ start(): void; pause(): void; resize(): void; dispose(): void; autoDriver?: { enabled: boolean; speed: number; resumeDelay: number; rampDurationMs: number; forceStop(): void; mouse?: { autoIntensity: number; takeoverDuration: number } }; output?: { simulation?: { options: Record<string, unknown>; resize(): void } } } | null>(null);
  ...
  useEffect(() => {
    ...
    // ─── AutoDriver ────────────────────────────────────────────────────────────
    interface IManager { lastUserInteraction: number; }
    class AutoDriver {
      mouse: MouseClass;
      manager: IManager;
      enabled: boolean;
      ...
      constructor(mouse: MouseClass, manager: IManager, opts: { enabled: boolean; speed: number; resumeDelay?: number; rampDuration?: number }) {
        this.mouse = mouse;
    ...
    class ShaderPass {
      ...
      init() {
        ...
        if (this.uniforms && this.props.material) {
          this.material = new THREE.RawShaderMaterial(this.props.material as ConstructorParameters<typeof THREE.RawShaderMaterial>[0]);
    ...
    class Advection extends ShaderPass {
      ...
      override init() {
        ...
        this.line = new THREE.LineSegments(geo, new THREE.RawShaderMaterial({
          vertexShader: line_vert, fragmentShader: advection_frag, uniforms: this.uniforms!,
        }));
        this.scene!.add(this.line);
      }
      updateSim({ dt, isBounce, BFECC }: { dt: number; isBounce: boolean; BFECC: boolean }) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      updateForce(props: { cursor_size: number; mouse_force: number; cellScale: THREE.Vector2 }) {
    ...
    class Divergence extends ShaderPass {
      ...
      updateDiv({ vel }: { vel: THREE.WebGLRenderTarget }) {
    ...
    type FBOs = {
      vel_0: THREE.WebGLRenderTarget; vel_1: THREE.WebGLRenderTarget;
      vel_viscous0: THREE.WebGLRenderTarget; vel_viscous1: THREE.WebGLRenderTarget;
      div: THREE.WebGLRenderTarget; pressure_0: THREE.WebGLRenderTarget; pressure_1: THREE.WebGLRenderTarget;
    };
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        ['vel_0','vel_1','vel_viscous0','vel_viscous1','div','pressure_0','pressure_1'].forEach((key) => {
      ...
      createShaderPass() {
        ...
        this.divergencePass = new Divergence({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.vel_viscous0, dst: this.fbos.div, dt: this.options.dt });
        this.poissonPass = new Poisson({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.div, dst: this.fbos.pressure_1, dst_: this.fbos.pressure_0 });
      ...
      update() {
        ...
        this.advection.updateSim({ dt: this.options.dt, isBounce: this.options.isBounce, BFECC: this.options.BFECC });
        ...
        if (this.options.isViscous) vel = this.viscousPass.updateViscous({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt });
        this.divergencePass.updateDiv({ vel });
    ...
    class WebGLManager {
      ...
      constructor(props: { $wrapper: HTMLElement }) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this, { enabled: autoDemo, speed: autoSpeed, resumeDelay: autoResumeDelay, rampDuration: autoRampDuration });
