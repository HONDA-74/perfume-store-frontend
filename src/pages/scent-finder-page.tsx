...
function OptionBtn({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="relative border px-7 py-5 text-start transition-all duration-200"
      style={{
  ...
  return (
    <div>
      ...
      {productQuery.isLoading ? (
        <div className="aspect-[4/5] animate-pulse bg-white/[0.04]" role="status" aria-label={t('ai.loadingRecommendation')} />
      ) : productQuery.data ? (
        <ProductCard product={productQuery.data} />
      ) : (
        <p className="border border-white/[0.06] p-5 text-xs text-white/35">{t('ai.unavailable')}</p>
      )}
    </div>
  );
}
...
export function ScentFinderPage() {
    ...
    if (step < FREE_TEXT_STEP) {
      ...
      if (!isAuthenticated) {
        toast.error(t('ai.loginRequired'), {
          action: { label: t('ai.loginAction'), onClick: () => navigate(ROUTES.auth.login, { state: { returnTo: location.pathname } }) },
        });
        return;
      }
        ...
        { message },
        {
          ...
          onError: (error) => {
            if (isNormalizedApiError(error) && error.status === 401) {
              toast.error(t('ai.loginRequired'), { action: { label: t('ai.loginAction'), onClick: () => navigate(ROUTES.auth.login, { state: { returnTo: location.pathname } }) } });
            } else {
  ...
  }, [step, isFreeTextStep, isAuthenticated, t, navigate, location.pathname, buildPromptMessage, sendChatMutation]);
          ...
          <div key={step} style={{ animation: 'fadeUp 0.35s ease both' }}>
            ...
            {question && !isFreeTextStep && (
              <>
                <h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 'clamp(1.4rem, 2.5vw, 2rem)', color: 'rgba(243,242,245,0.78)', marginBottom: '36px', maxWidth: '480px', lineHeight: 1.25 }}>
                  {t(`ai.questions.${question.id === 'fragranceFamily' ? 'family' : question.id}`)}
                </h2>
                <div className="flex flex-wrap gap-3 mb-14">
                  {question.options.map(opt => (
                    <OptionBtn
                      key={opt.value}
                      label={t(`ai.options.${opt.value}`)}
              ...
              <div className="max-w-xl">
                ...
                {sendChatMutation.isError && (
                  <div className="mb-6 p-4 border border-red-500/20 bg-red-500/5 flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-red-300 text-sm mb-1 font-medium">{t('ai.failed')}</p>
                      <p className="text-red-400/70 text-xs">
                        {t('ai.genericError')}
                      </p>
                      <p className="text-red-400/50 text-xs mt-2">{t('ai.adjust')}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-4">
              {step > 0 && (
                ...
                >
                  <ArrowLeft size={12} className="directional-icon" /> {t('common.back')}
                </button>
              )}
              <button
                onClick={handleNext}
              ...
              >
                {sendChatMutation.isPending ? (
                  <>
                    <Loader2 size={12} className="animate-spin" /> {t('ai.finding')}
                  </>
                ) : isFreeTextStep ? (
                  <>
                    <Sparkles size={12} /> {t('ai.findMine')}
                  </>
                ) : (
                  <>
                    {t('common.continue')} <ArrowRight size={12} className="directional-icon" />
                  </>
                )}
              </button>
            </div>
        ...
        <div className="max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 pb-20">
          <div className="mb-10" style={{ animation: 'fadeUp 0.4s ease both' }}>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', fontWeight: 300, color: 'rgba(243,242,245,0.22)', maxWidth: '500px' }}>
              {quiz.userDescription && t('ai.guided')}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              {t('landing.explore')} <ArrowRight size={12} className="directional-icon" />
            </Link>
            <button
            ...
            >
              <RefreshCw size={12} /> {t('ai.restart')}
            </button>
          </div>
        </div>
      )}
