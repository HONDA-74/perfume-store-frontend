  ...
  return (
    <div>
      <header className="mb-8 flex items-start justify-between gap-4">
        <div><p className="mb-1.5 text-[9px] font-medium uppercase tracking-[0.2em] text-[#D4C3A3]/40">Account</p><h1 className="font-serif text-[clamp(1.5rem,2.5vw,2rem)] text-white/85">Addresses</h1></div>
        {!dialog && <button onClick={() => setDialog({ kind: 'add' })} className="flex h-10 shrink-0 items-center gap-2 border border-white/10 px-5 text-[10px] uppercase tracking-[0.1em] text-white/45 transition-colors hover:border-white/20"><Plus size={13} /> Add Address</button>}
      </header>
      ...
      {addresses.isError ? <EmptyAddress title="Addresses could not be loaded" subtitle="Please try again." /> : !items.length ? <EmptyAddress title="No addresses saved" subtitle="Add a delivery address for a seamless checkout experience." /> : (
        <div className="space-y-4">{items.map((address) => (
          <article key={address.id} className={`border bg-[#121115] ${address.isDefault ? 'border-[#D4C3A3]/20 border-l-2 border-l-[#D4A017]' : 'border-white/[0.06] border-l-2 border-l-transparent'}`}>
            <div className="p-5">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2"><p className="text-xs font-medium text-white/75">{address.recipientName}</p>{address.isDefault && <span className="flex items-center gap-1 rounded-sm bg-[#D4A017]/10 px-1.5 py-0.5 text-[8px] font-medium uppercase tracking-[0.12em] text-[#D4A017]"><Star size={8} fill="currentColor" /> Default</span>}</div>
                <div className="flex shrink-0 items-center">
                  {!address.isDefault && <button onClick={() => updateAddress.mutate({ id: address.id, payload: { isDefault: true } })} aria-label="Set as default" className="p-2 text-white/25 transition-colors hover:text-white/55"><Star size={14} /></button>}
                  <button onClick={() => { setDialog({ kind: 'edit', address }); setConfirmDelete(null); }} aria-label="Edit address" className="p-2 text-white/25 transition-colors hover:text-white/55"><Pencil size={14} /></button>
                  <button onClick={() => { setConfirmDelete(address.id); setDialog(null); }} aria-label="Delete address" className="p-2 text-white/25 transition-colors hover:text-red-300/70"><Trash2 size={14} /></button>
                </div>
              </div>
