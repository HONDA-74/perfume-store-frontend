  ...
  return (
    <div className={mobile ? 'space-y-7' : 'grid grid-cols-2 gap-x-8 gap-y-6 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-7'}>
      <div>
        ...
        <div className={mobile ? '' : 'max-h-32 overflow-y-auto pr-2'}>{brands.map((item) => <Choice key={item.id} label={item.name} checked={brandId === item.id} onChange={() => update('brandId', brandId === item.id ? undefined : item.id)} />)}</div>
      </div>
      ...
      <div>
        ...
        <div className={mobile ? '' : 'max-h-32 overflow-y-auto pr-2'}>{categories.map((item) => <Choice key={item.id} label={item.name} checked={categoryId === item.id} onChange={() => update('categoryId', categoryId === item.id ? undefined : item.id)} />)}</div>
      </div>
      ...
      <div>
        ...
        <input aria-label="Maximum price" type="range" min="0" max={PRICE_MAX} step="10" value={maxPrice} onChange={(event) => update('maxPrice', Number(event.target.value) < PRICE_MAX ? event.target.value : undefined)} className="h-4 w-full accent-[#D4A017]" />
        <p className="mt-2 text-[10px] font-light text-white/30">Up to <span className="text-[#D4A017]">${maxPrice}</span></p>
      </div>
    ...
    <div className="min-h-screen bg-[#0B0A0C] text-[#F3F2F5]">
        ...
        <div className="mx-auto max-w-[1440px] px-6 lg:px-12">
          <div className="flex h-12 items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              {activeCount > 0 && <button onClick={reset} className="hidden items-center gap-1.5 text-[9px] font-medium uppercase tracking-[0.12em] text-[#D4A017]/80 lg:flex"><X size={9} />{activeCount} Active</button>}
              <label className="sr-only" htmlFor="shop-sort">Sort products</label>
              <select id="shop-sort" value={sort} onChange={(event) => update('sort', event.target.value)} className="h-8 cursor-pointer appearance-none border border-white/[0.08] bg-transparent py-0 pl-3 pr-8 text-[10px] tracking-[0.06em] text-white/55 outline-none focus:border-[#D4C3A3]/50">
                {SORT_OPTIONS.map((option) => <option key={option.value} value={option.value} className="bg-[#121115]">{option.label}</option>)}
              </select>
            </div>
          </div>
          <div className="-mt-1 flex items-center justify-between pb-2">
            <button onClick={() => setDesktopOpen((value) => !value)} className="hidden h-7 items-center gap-2 text-[10px] font-medium uppercase tracking-[0.15em] text-white/40 transition hover:text-white/70 lg:flex" aria-expanded={desktopOpen}>{desktopOpen ? <Minus size={10} /> : <Plus size={10} />}Filters{activeCount > 0 && !desktopOpen && ` · ${activeCount}`}</button>
            <button onClick={() => setMobileOpen(true)} className="flex h-7 items-center gap-2 text-[10px] font-medium uppercase tracking-[0.12em] text-white/50 lg:hidden"><SlidersHorizontal size={11} />Filters{activeCount > 0 && ` (${activeCount})`}</button>
          </div>
        </div>
      </div>
      ...
      <div className={`hidden overflow-hidden border-b border-white/[0.05] bg-[#0D0C10] transition-[max-height,opacity] duration-300 lg:block ${desktopOpen ? 'max-h-[24rem] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="mx-auto max-w-[1440px] px-12 py-7">
          ...
          {activeCount > 0 && <div className="mt-5 flex justify-end border-t border-white/[0.05] pt-4"><button onClick={reset} className="flex items-center gap-1.5 text-[9px] font-medium uppercase tracking-[0.15em] text-white/35 hover:text-white/65"><X size={9} />Reset Filters</button></div>}
        </div>
      </div>
        ...
        {products.isLoading ? <PageLoader /> : products.isError ? <ErrorState className="min-h-[50vh]" onRetry={() => products.refetch()} message="We couldn't load the fragrance collection." /> : !productData?.items.length ? <EmptyState className="min-h-[50vh]" title="No Fragrance Found" message="Try adjusting your filters to discover another expression of scent." actionLabel="Reset Filters" onAction={reset} /> : (
          <>
            ...
            {totalPages > 1 && <nav className="mt-14 flex items-center justify-center gap-2" aria-label="Product pages">
              <button onClick={() => changePage(page - 1)} disabled={page === 1} className="flex h-9 w-9 items-center justify-center border border-white/[0.08] text-white/40 hover:border-white/20 hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-25" aria-label="Previous page"><ChevronLeft size={14} /></button>
              ...
              <button onClick={() => changePage(page + 1)} disabled={page === totalPages} className="flex h-9 w-9 items-center justify-center border border-white/[0.08] text-white/40 hover:border-white/20 hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-25" aria-label="Next page"><ChevronRight size={14} /></button>
            </nav>}
          </>
        )}
      </div>
